...
// black box): only the *inputs* are private, the *logic* is auditable. It is also
// deterministic, which is the property that makes the signed grade checkable — anyone
// with the same inputs and the same image digest can recompute the identical result.
// Keeping it a plain weighted formula rather than a model with hidden state is a
// deliberate product choice, not a limitation.
//
...
// so the result is exactly reproducible across machines — no floating point anywhere.
package scoring
...
// jurisdictionScoreBps is a static, published table. Higher = lower risk. Unknown
// jurisdictions get jurisdictionDefaultBps. Values are a governance parameter, not a
// measurement — they are disclosed rather than presented as empirical.
...
// Input is the buyer's payment-history data to be scored. It may arrive as plaintext over TLS
// or ECIES-encrypted to the service's published key. Amounts are unitless but must share the
// same unit.
type Input struct {
	// InvoiceID binds this scoring request to a specific on-chain invoice (0x-prefixed
	// 32-byte hex). It is NOT a scoring factor — the model ignores it — but the service
	// echoes it into the signed result so CifraAttestationNFT can verify the grade was
	// produced for exactly this invoice (audit finding H1), which is what stops a grade
	// being replayed onto a different, riskier invoice.
	...
	// Jurisdiction: ISO-3166 alpha-2 code of the buyer, looked up in the static table below.
	JurisdictionCode string `json:"jurisdictionCode"`

	// JurisdictionScoreBps optionally overrides the static table (higher = lower risk) with a
	// value the caller sourced from an on-chain jurisdiction oracle. 0 = use the table.
	// This is the one scoring input that is not buyer-private — a country's region is public —
	// which is why it is the only term that can be sourced externally at all.
	JurisdictionScoreBps uint32 `json:"jurisdictionScoreBps,omitempty"`

	// --- Provenance of the private inputs (optional) ---
	//
	// When ProvenanceCommitment is set, the service recomputes
	// keccak256(ProvenanceCanonical(in) ‖ salt) and REFUSES TO SCORE unless it matches AND a
	// trusted data source signed that commitment. So a funder can check the grade was computed
	// on data the source vouched for, without the data ever being disclosed to them or put
	// on-chain.
	//
	// The salt is private: the inputs are low-entropy enough to brute-force from a bare
	// commitment, so publishing the commitment alone would leak them.
	ProvenanceCommitment string `json:"provenanceCommitment,omitempty"` // 0x-prefixed 32-byte hex
	ProvenanceSalt       string `json:"provenanceSalt,omitempty"`       // 0x-prefixed hex, kept private
	// ProvenanceSignature is the data source's EIP-191 signature over the commitment bytes.
	// This is what replaced Flare's FDC Web2Json anchor: rather than an attestation network
	// vouching for an HTTP response, the source signs its own data. For a specific customer's
	// private payment history — which no public API serves — only the source can authenticate
	// it, so this is arguably the more appropriate model as well as the portable one.
	ProvenanceSignature string `json:"provenanceSignature,omitempty"` // 0x-prefixed 65-byte hex
}

// ProvenanceCanonical is the deterministic serialization of the PRIVATE buyer inputs that the
// provenance commitment covers. The data source and this service must agree on it byte for byte
// (same lesson as the on-chain invoice commitment): fixed field order, pipe-delimited.
// keccak256(ProvenanceCanonical(in) ‖ salt) == the commitment the source signed.
...
// Result is the scored output. Only this is returned and signed — never the Input, which the
// service does not persist or log.
type Result struct {
...
func Score(in Input) (Result, error) {
	...
	// Prefer a caller-supplied jurisdiction score (from an on-chain oracle) over the table.
