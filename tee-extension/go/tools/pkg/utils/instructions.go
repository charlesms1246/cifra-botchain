...
// SendScore sends a CIFRA/SCORE instruction carrying ECIES-encrypted buyer data.
// Mirrors SendSign. Returns (instructionId, txHash).
func SendScore(s *support.Support, instructionSenderAddress common.Address, encryptedBuyerData []byte) (common.Hash, common.Hash, error) {
	sender, err := sign.NewInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
		return common.Hash{}, common.Hash{}, errors.Errorf("failed to bind contract: %s", err)
	}
	...
	tx, err := sender.SendScore(opts, encryptedBuyerData)
	if err != nil {
		reason := fccutils.DecodeRevertReason(err)
		if reason == "" {
			parsed, _ := sign.InstructionSenderMetaData.GetAbi()
			if parsed != nil {
				callData, packErr := parsed.Pack("sendScore", encryptedBuyerData)
				if packErr == nil {
					from := crypto.PubkeyToAddress(s.Prv.PublicKey)
					reason = fccutils.SimulateAndDecodeRevert(
						s.ChainClient, from, instructionSenderAddress, DefaultFee, callData,
		...
		if reason != "" {
			return common.Hash{}, common.Hash{}, errors.Errorf("failed to send sendScore: %s (revert reason: %s)", err, reason)
		}
		return common.Hash{}, common.Hash{}, errors.Errorf("failed to send sendScore: %s", err)
	}
	...
	if receipt.Status != 1 {
		return common.Hash{}, common.Hash{}, errors.Errorf("sendScore transaction failed with status: %d", receipt.Status)
	}
	if len(receipt.Logs) == 0 {
		return common.Hash{}, common.Hash{}, errors.New("no logs found in receipt")
	}
