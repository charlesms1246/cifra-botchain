...
contract InstructionSender {
    ...
    /// @notice Operation type for Cifra invoice risk scoring.
    // forge-lint: disable-next-line(unsafe-typecast)
    bytes32 public constant OP_TYPE_CIFRA = bytes32("CIFRA");

    /// @notice Command for the SCORE action (privately scores buyer data, returns a signed grade).
    // forge-lint: disable-next-line(unsafe-typecast)
    ...
    /// @notice Privately score an invoice's buyer risk inside the TEE.
    /// @param _encryptedBuyerData ECIES-encrypted (to the TEE public key) buyer payment-history
    ///        JSON. The TEE node decrypts it in the enclave, applies Cifra's v1 model, and returns
    ///        an ABI-encoded (bytes32 grade, uint256 riskScoreBps, uint256 discountRateBps) result
    ///        signed by the TEE identity. Raw buyer data never leaves the enclave.
    function sendScore(bytes calldata _encryptedBuyerData) external payable {
        address[] memory teeIds = TEE_MACHINE_REGISTRY.getRandomTeeIds(_getExtensionId(), 1);
        address[] memory cosigners = new address[](0);

        ITeeExtensionRegistry.TeeInstructionParams memory params = ITeeExtensionRegistry.TeeInstructionParams({
            opType: OP_TYPE_CIFRA,
            opCommand: OP_COMMAND_SCORE,
            message: _encryptedBuyerData,
            cosigners: cosigners,
            cosignersThreshold: 0,
            claimBackAddress: msg.sender
        });
